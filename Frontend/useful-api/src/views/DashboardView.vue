<template>
    <div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
            <div v-for="module in adminStore.modules" :key="module.id"
                class="p-5 bg-green-100 rounded-xl border-1 border-dashed border-black" :class="[
                    'hover:scale-110 hover:cursor-pointer transition delay-100 duration-150', backgroundcolor()
                ]">
                <div class="flex justify-between gap-2 mb-5">
                    <div @click="activateModule(module.id)" class="text-sm px-2 py-1 text-white bg-blue-500 rounded-md">
                        Activate
                    </div>
                    <svg @click="desactivateModule(module.id)" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                        class="w-6" viewBox="0 0 100 100">
                        <path fill="#f37e98"
                ...
                <input v-model="module.name" id="category"
                    class="block w-full mb-4 outline-1 focus:outline-2 focus:outline-gray-600 px-2 py-1 rounded-md outline-gray-400 resize-none"
                    type="text" name="name">
            </div>
        </div>
    </div>
